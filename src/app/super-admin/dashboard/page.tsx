import Link from "next/link";
import {
  Activity,
  ArrowUpRight,
  Banknote,
  KeyRound,
  Server,
  ShieldCheck,
  UserCog,
  WalletCards,
} from "lucide-react";

import { createAdminClient } from "@/lib/supabase/admin";
import { getDashboardStats, getWeeklyRevenue } from "@/lib/db";

export const revalidate = 0; // Live dashboard reports

export default async function SuperAdminDashboardPage() {
  const supabase = createAdminClient();

  const [stats, weeklySales, staffResult, auditResult] = await Promise.all([
    getDashboardStats(),
    getWeeklyRevenue(),
    supabase.from("profiles").select("*").in("role", ["admin", "super_admin"]),
    supabase.from("analytics_events").select("*").order("created_at", { ascending: false }).limit(5),
  ]);

  const staffList = staffResult.data ?? [];
  const auditList = auditResult.data ?? [];

  const grossRevenue = stats.total_revenue;
  const netRevenue = stats.total_revenue * 0.81; // Deduct simulated tax/platform fee bounds
  const refundExposure = stats.total_revenue * 0.02; // Simulated refund exposure allocation

  const executiveMetrics = [
    { 
      label: "Gross revenue", 
      value: `EUR ${grossRevenue.toLocaleString("en-US", { minimumFractionDigits: 0 })}`, 
      change: "Store sales gross", 
      icon: WalletCards 
    },
    { 
      label: "Net revenue", 
      value: `EUR ${netRevenue.toLocaleString("en-US", { minimumFractionDigits: 0 })}`, 
      change: "Est. Net profit", 
      icon: Banknote 
    },
    { 
      label: "Refund exposure", 
      value: `EUR ${refundExposure.toLocaleString("en-US", { minimumFractionDigits: 0 })}`, 
      change: "2% buffer model", 
      icon: Activity 
    },
    { 
      label: "Active staff", 
      value: `${staffList.length}`, 
      change: "Security roles", 
      icon: UserCog 
    },
  ];

  // Normalize chart details
  const maxSales = Math.max(...weeklySales.map((s) => s.value), 1);
  const chartSales = weeklySales.map((s) => ({
    day: s.day,
    amount: `€${(s.value / 1000).toFixed(1)}K`,
    percentage: (s.value / maxSales) * 100,
  }));

  return (
    <div className="pb-12">
      <div className="mb-8 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="mb-3 text-[9px] font-bold uppercase tracking-[0.22em] text-red-500">Full authority workspace</p>
          <h1 className="text-3xl font-bold uppercase tracking-[0.12em] text-white">Super Admin Command Center</h1>
          <p className="mt-3 max-w-3xl text-xs uppercase tracking-[0.12em] text-neutral-400">
            Complete visibility across finance, commerce, staff access, security, and store infrastructure.
          </p>
        </div>
        <div className="inline-flex items-center gap-2 rounded-md border border-red-500/20 bg-red-500/5 px-4 py-3 text-[9px] font-bold uppercase tracking-[0.14em] text-red-400">
          <ShieldCheck className="h-4 w-4" />All permissions active
        </div>
      </div>

      {/* Metrics Row */}
      <section className="mb-8 border border-neutral-800 rounded-xl overflow-hidden bg-black/60 backdrop-blur-md grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 divide-y xl:divide-y-0 xl:divide-x divide-neutral-800">
        {executiveMetrics.map((metric) => (
          <div key={metric.label} className="p-6 hover:bg-neutral-900/10 transition-colors duration-200 group">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-[9px] font-bold uppercase tracking-[0.18em] text-neutral-500 group-hover:text-neutral-400 transition-colors">{metric.label}</p>
              <metric.icon className="h-4 w-4 text-neutral-600 group-hover:text-red-500 transition-colors" />
            </div>
            <p className="text-2xl font-bold tracking-tight text-white">{metric.value}</p>
            <p className="mt-2 text-[9px] font-bold uppercase tracking-[0.14em] text-neutral-400">{metric.change}</p>
          </div>
        ))}
      </section>

      <div className="mb-8 grid grid-cols-1 gap-6 xl:grid-cols-[1.5fr_1fr]">
        
        {/* Weekly Revenue Graph */}
        <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-6">
          <div className="mb-7 flex items-center justify-between">
            <div>
              <h2 className="text-xs font-bold uppercase tracking-[0.18em] text-white">Revenue control</h2>
              <p className="mt-2 text-[9px] uppercase tracking-[0.15em] text-neutral-500">Gross, tax, discounts, and refund exposure</p>
            </div>
            <Link href="/super-admin/sales" className="text-[9px] font-bold uppercase tracking-[0.14em] text-red-400">Open finance</Link>
          </div>
          <div className="flex h-64 items-end gap-3">
            {chartSales.map((entry) => (
              <div key={entry.day} className="flex h-full min-w-0 flex-1 flex-col justify-end gap-3">
                <div className="flex flex-1 items-end overflow-hidden rounded-t-sm border border-neutral-800 bg-black">
                  <div className="w-full border-t border-red-500/70 bg-red-500/10" style={{ height: `${entry.percentage}%` }} />
                </div>
                <span className="text-center text-[8px] font-bold uppercase text-neutral-500">{entry.day}</span>
              </div>
            ))}
          </div>
        </section>

        {/* System Health Stats */}
        <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-6">
          <h2 className="mb-5 text-xs font-bold uppercase tracking-[0.18em] text-white">System health</h2>
          {[
            ["Storefront", "Operational", "99.99%"],
            ["Checkout API", "Operational", "128ms"],
            ["Database", "Healthy", "34ms"],
            ["Cache Server", "Healthy", "8ms"],
          ].map(([service, status, metric]) => (
            <div key={service} className="flex items-center gap-3 border-b border-neutral-800 py-4 last:border-0">
              <Server className="h-4 w-4 text-emerald-400" />
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold uppercase tracking-[0.13em] text-white">{service}</p>
                <p className="mt-1 text-[8px] uppercase tracking-[0.14em] text-emerald-400">{status}</p>
              </div>
              <span className="text-[9px] text-neutral-500">{metric}</span>
            </div>
          ))}
        </section>
      </div>

      <div className="grid grid-cols-1 gap-6 xl:grid-cols-2">
        
        {/* Privileged Staff Accounts */}
        <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-6">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="text-xs font-bold uppercase tracking-[0.18em] text-white">Staff access</h2>
            <Link href="/super-admin/admins" className="text-[9px] font-bold uppercase text-red-400">Manage</Link>
          </div>
          {staffList.slice(0, 3).map((member) => (
            <div key={member.email} className="flex items-center gap-4 border-b border-neutral-800 py-4 last:border-0">
              <KeyRound className="h-4 w-4 text-red-500" />
              <div className="min-w-0 flex-1">
                <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-white">{member.name}</p>
                <p className="mt-1 text-[9px] text-neutral-500">{member.role}</p>
              </div>
              <span className="text-[8px] uppercase text-emerald-400">ACTIVE</span>
            </div>
          ))}
        </section>

        {/* Live Security Audit Log */}
        <section className="rounded-lg border border-neutral-800 bg-neutral-900 p-6">
          <div className="mb-5 flex items-center justify-between">
            <h2 className="text-xs font-bold uppercase tracking-[0.18em] text-white">Security activity</h2>
            <Link href="/super-admin/audit" className="text-[9px] font-bold uppercase text-red-400">Full audit</Link>
          </div>
          {auditList.slice(0, 3).map((event) => (
            <div key={event.id} className="flex items-start gap-4 border-b border-neutral-800 py-4 last:border-0">
              <ArrowUpRight className="mt-0.5 h-4 w-4 text-red-500" />
              <div>
                <p className="text-[10px] font-semibold text-white">{event.event}</p>
                <p className="mt-1 text-[8px] uppercase tracking-[0.12em] text-neutral-500">
                  {new Date(event.created_at).toLocaleString()}
                </p>
              </div>
            </div>
          ))}
          {auditList.length === 0 && (
            <p className="text-[9px] uppercase tracking-widest text-neutral-600 mt-6">No recent security events registered.</p>
          )}
        </section>
      </div>
    </div>
  );
}
